...
let NtlmProxyMitm = class NtlmProxyMitm {
    ...
    onResponse(ctx, callback) {
        ...
        if (context && context.canStartAuthHandshake(targetHost)) {
            ...
            if (authMode === 0 /* NotApplicable */) {
                return callback();
            }
            if (authMode === 1 /* NotSupported */) {
            ...
            if (authMode === 4 /* Negotiate */) {
                self._debug.log("Received 401 with Negotiate in www-authenticate header. Starting handshake.");
                if (useSso) {
                    context.winSso = self._winSsoFacadeFactory.create("Negotiate", ctx.proxyToServerRequestOptions.host, context.peerCert);
                }
            ...
            if (authMode === 3 /* NTLM */) {
                ...
                if (useSso) {
                    context.winSso = self._winSsoFacadeFactory.create("NTLM", ctx.proxyToServerRequestOptions.host, context.peerCert);
                }
